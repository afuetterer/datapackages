...
def read_tables(name):
    ...
    print("################")
    tables = glob.glob("soep-core/metadata/*/*/%s" % name)
    tables += glob.glob("soep-core/metadata/%s" % name)
    print(tables)
    print(type(tables))
    # print(tables.pop())
...
def import_tables():
    tables = OrderedDict(
        questionnaires=pd.read_csv("soep-core/metadata/questionnaires.csv"),
        questions=read_tables("questions.csv"),
        answers=read_tables("answers.csv"),
    )

    print(type(tables))
    print(type(tables["questionnaires"]))
    print(type(tables["questions"]))
